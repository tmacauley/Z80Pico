...
INLINE void WrZ80(zword A, byte V)
{
  if (Page[A >> 13] < ROM)
...
#define M_POP(Rg)                 \
  R->Rg.B.l = OpZ80(R->SPtr.W++); \
  R->Rg.B.h = OpZ80(R->SPtr.W++)
#define M_PUSH(Rg)               \
  WrZ80(--R->SPtr.W, R->Rg.B.h); \
  WrZ80(--R->SPtr.W, R->Rg.B.l)

#define M_CALL                   \
  J.B.l = OpZ80(R->PC.W++);      \
  J.B.h = OpZ80(R->PC.W++);      \
  WrZ80(--R->SPtr.W, R->PC.B.h); \
  WrZ80(--R->SPtr.W, R->PC.B.l); \
  R->PC.W = J.W;                 \
  JumpZ80(J.W)
...
#define M_JP                \
  ...
  JumpZ80(J.W)
#define M_JR                             \
  R->PC.W += (offset)OpZ80(R->PC.W) + 1; \
  JumpZ80(R->PC.W)
#define M_RET                     \
  R->PC.B.l = OpZ80(R->SPtr.W++); \
  R->PC.B.h = OpZ80(R->SPtr.W++); \
  JumpZ80(R->PC.W)
...
#define M_LDWORD(Rg)            \
  R->Rg.B.l = OpZ80(R->PC.W++); \
  R->Rg.B.h = OpZ80(R->PC.W++)
